...
static size_t replace(std::string &base, const std::string &src, const std::string &dst = "");
...
size_t replace(std::string &base, const std::string &src, const std::string &dst)
{
...
size_t args_parse(const std::string &args, std::unordered_map<std::string, std::string> &args_map, const std::string &delim)
{
	...
	while ((idx = args_text.find(delim)) != std::string::npos)
	{
		std::string str = args_text.substr(0, idx);
		args_text = args_text.substr(idx + delim.length());

		if ((idx = str.find("=")) != std::string::npos)
		{
			std::string key = ustd::string::trim(ustd::string::tolower(str.substr(0, idx)));
			if (args_map.find(key) == args_map.end())
			{
				args_map.insert(std::make_pair(key, str.substr(idx + 1)));
				args_count += 1;
			}
		}
	}

	if ((idx = args_text.find("=")) != std::string::npos)
	{
		std::string key = ustd::string::trim(ustd::string::tolower(args_text.substr(0, idx)));
		if (args_map.find(key) == args_map.end())
		{
			args_map.insert(std::make_pair(key, args_text.substr(idx + 1)));
			args_count += 1;
		}
	}
